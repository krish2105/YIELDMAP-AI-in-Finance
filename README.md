...
| Risk and anomalies | 1,738 of 120,346 flagged (1.44%); 86 areas scored | score in [0,100], monotone |
...
| Page weight | 207–210 KB of JavaScript brotli-compressed on most routes, 292 KB on a chart page; three.js on none of them | gated at 340 KB, and at zero three.js |
...
| [`docs/AI217_YIELDMAP_report.docx`](docs/AI217_YIELDMAP_report.docx) | The Term 4 report, generated from `docs/results/` by `scripts/build_report.py` |
| [`docs/AI217_YIELDMAP_deck.pptx`](docs/AI217_YIELDMAP_deck.pptx) | Twelve slides with speaker notes, generated by `scripts/build_deck.py` |
| [`docs/REDEPLOY_RUNBOOK.md`](docs/REDEPLOY_RUNBOOK.md) | Restore and redeploy the whole thing from a downloaded zip |

The report and the deck are generated rather than written, and both currently carry a **draft
stamp**: `scripts/guard_synthetic.py` blocks generated figures from reaching a submitted artefact,
and it blocks. Passing `--draft` is what allows generation, and it puts the label in three places a
reader cannot get past — a banner on the cover, a provenance column beside every model figure, and
a limitations section naming which numbers describe a stand-in. Load real data and regenerate: the
guard passes, and the provenance column disappears because it has nothing left to say.
