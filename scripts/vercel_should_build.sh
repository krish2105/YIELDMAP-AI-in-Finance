...
#
# Vercel runs this before every deployment. The exit codes are the platform's, and they read
# backwards from every other command-line tool:
#
#     exit 0  -> IGNORE the build (Vercel cancels the deployment)
#     exit 1  -> CONTINUE the build
#
# https://vercel.com/docs/project-configuration/vercel-json — "Exiting with code 0 ignores the
# build, while code 1 continues it." Getting this the natural way round means every commit the
# gate judges relevant is silently skipped, which is exactly what happened here: deployments came
# back CANCELED, not failed, so nothing looked broken.
#
# Without this gate, the nightly ingest's own commits — probe results, archived source pages, none
# of which the browser ever sees — each triggered a full Next.js build. On a Hobby plan those
# minutes are finite, and the deployment they produce is byte-identical to the one before it.
#
