...
The table below is generated from the probe artefact by
`scripts/render_probe_table.py`, and CI fails if it drifts from it. It was prose until
2026-09-07, and by then it had drifted: it called `gateway.dubailand.gov.ae` reachable while the
artefact recorded `ok: false` and a 404, because "reachable" was being used to mean *the host
responded* in one place and *an address served what was asked for* in the other. The rows now
carry counts and observed statuses only; the reading of them is below, where it is visibly a
reading.

<!-- probe:begin -- generated by scripts/render_probe_table.py; do not edit by hand -->

Measured from a `github-actions` runner at 2026-09-06T20:10:03+00:00, from `docs/results/source_probe.json`. 4 of 13 addresses answered. Its `verdict` field says **reachable**, but this artefact predates the catalogue-aware verdict and was computed under the old rule, which counted any address answering — reference documentation included. The next nightly run replaces it.

| Host | Addresses tried | Answered | What came back |
|---|---|---|---|
| `dubailand.gov.ae` | 4 | 4 | 200 (×4) |
| `gateway.dubailand.gov.ae` | 1 | 0 | 404 |
| `www.centralbank.ae` | 1 | 0 | 403 |
| `www.dubaipulse.gov.ae` | 7 | 0 | ConnectTimeout (×7) |

<!-- probe:end -->

What that says, in words:

- **`dubailand.gov.ae`** serves its public pages normally. The open-data, services and
  rental-index pages all answer 200.
- **`www.dubaipulse.gov.ae`** — the host that publishes the bulk CSVs — times out at connect on
  every address tried. A timeout rather than a refusal or a 403 reads as a block on datacenter
  address ranges, not a wrong path.
- **`gateway.dubailand.gov.ae`** answers, but 404 at the address tried. The host is up; no
  address on it served data. It is not a route to the datasets.
- **`www.centralbank.ae`** answers 403 to an automated client.
